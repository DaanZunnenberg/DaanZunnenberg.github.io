(function () {
  ...
  //
  // Rendered with KaTeX's own katex.render(source, node, opts) — called
  // directly rather than through the text-scanning auto-render helper, so
  // there's no dependence on delimiter-matching inside injected text nodes.
  // `tex` below is raw TeX source (no \[ \] wrapper) set once via
  // textContent before rendering, so the source is still a plain string
  // literal with no HTML parsing involved anywhere — no injection surface.
  //
  ...
  var ITEMS = [
    ...
    },
    {
      ...
      prose: "When a process has increments controlled jointly by two metrics, it splits into two pieces, each governed by a single metric on its own, so the total chaining cost never exceeds the sum of the two separate costs.",
    ...
    },
    {
      ...
      prose: "A metric space satisfies a growth condition of order alpha when balls of radius epsilon cannot contain too many epsilon-separated points — a purely combinatorial condition that lets the chaining functional be estimated straight from volume, without building an admissible sequence by hand.",
  ...
  function renderSlide(item) {
    ...
    try {
      window.katex.render(item.tex, eqBox, { displayMode: true, throwOnError: false, strict: false });
    } catch (e) {
      eqBox.textContent = ""; // fail quietly, never fall back to raw TeX text
    }
    return wrap;
  }
